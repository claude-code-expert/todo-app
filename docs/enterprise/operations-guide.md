...
| 영역          | MVP                | 팀                 | SaaS                   |
| ------------- | ------------------ | ------------------ | ---------------------- |
| **호스팅**    | Vercel             | Vercel             | Vercel + CDN           |
| **DB**        | Neon (단일)        | Neon (환경별 분리) | Neon + 커넥션 풀링     |
| **모니터링**  | Vercel Logs        | Sentry + Logs      | Datadog / Grafana      |
| **알림**      | 이메일             | Slack 연동         | PagerDuty + 온콜       |
| **CI/CD**     | Git push 자동 배포 | GitHub Actions CI  | 승인 게이트 + 스테이징 |
| **배포 전략** | 직접 배포          | Preview 배포       | Blue-Green / Canary    |
| **보안**      | 기본 HTTPS         | 의존성 스캔        | SAST + DAST            |
| **IaC**       | 수동 설정          | 환경 변수 관리     | Terraform              |
...
오픈소스로 벤더 종속을 피하거나, 자체 호스팅 환경에서 운영할 때 적합하다. Grafana Cloud를 사용하면 관리 부담 없이 시작할 수 있다.
...
| **Datadog**      | 대규모 APM + 인프라       | 우수 (APM + RUM + 로그) | 있음 (RUM)    | 14일 체험         | APM $15/호스트/월 |
...
SaaS 고도화 단계. 인프라 상태가 모두 Git에 정의되고, 배포와 롤백이 완전히 자동화된다. 7.2절에서 GitOps를 다룬다.
...
SaaS 고객이 "이 소프트웨어에 어떤 의존성이 포함되어 있는가?"를 요구할 수 있다.
...
| **DB-per-tenant**     | 높음      | 높음 | 높음   | 완전 격리 필요 시           |
